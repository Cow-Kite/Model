...
class GIN(torch.nn.Module):
    def __init__(self):
        super().__init__()
        self.conv1 = GINConv(nn.Linear(dataset.num_node_features,16), eps=0)
        self.conv2 = GINConv(nn.Linear(16, dataset.num_classes), eps=0)

    def forward(self, data):
        x, edge_index = data.x, data.edge_index

        #x = self.embedding(x)#embedding 추가
